...
const statePath = path.join(vault, '.claude', 'plugin-doc-state.json');
const communityPluginsPath = path.join(vault, '.obsidian', 'community-plugins.json');
const indexScript = path.join(vault, '.claude', 'generate_plugin_indexes.js');
const args = new Set(process.argv.slice(2));
const fullRebuild = args.has('--full');
const forceRefresh = args.has('--refresh-only');
...
function slugAlias(name) {
  return `Obsidian ${name}`;
}

function makeFrontmatter(title, aliases, tags) {
  return `---\ntitle: ${title}\naliases:\n${yamlList(aliases)}\ntags:\n${yamlList(tags)}\ncategory: 插件使用经验\ncreated: ${today}\nupdated: ${today}\nstatus: 常用\n---`;
}

function inferCategory(manifest) {
  if (categoryMap[manifest.id]) return categoryMap[manifest.id];
  const hay = `${manifest.name || ''} ${manifest.description || ''} ${manifest.id || ''}`.toLowerCase();
  if (/(task|kanban|calendar|planner|recent)/.test(hay)) return '任务与工作流管理';
  if (/(dataview|template|templater|quickadd|query|metadata|automation)/.test(hay)) return '查询、模板与自动化';
  if (/(chart|draw|media|mind|timeline|annot)/.test(hay)) return '可视化、图形与媒体';
  if (/(editor|cursor|emoji|pair|codemirror|table)/.test(hay)) return '编辑器与输入增强';
  if (/(word|dictionary|footnote|write|translation)/.test(hay)) return '写作辅助与语言工具';
  if (/(ai|claude|gpt|copilot|assistant)/.test(hay)) return 'AI 与智能助手';
  return '插件增强';
}

function inferType(manifest, category) {
  if (typeMap[manifest.id]) return typeMap[manifest.id];
  return ['查询、模板与自动化', '任务与工作流管理'].includes(category) ? 'template' : 'example';
}

function linkLine(folderName, baseName, type) {
  const guide = `[[AI笔记/插件使用经验/${folderName}/${baseName}插件使用指南]]`;
  const cheat = `[[AI笔记/插件使用经验/${folderName}/${baseName}插件速查表]]`;
  const thirdName = type === 'template' ? `${baseName}${baseName === 'Tasks' ? '任务总览模板' : '模板'}` : `${baseName}示例页`;
  const third = `[[AI笔记/插件使用经验/${folderName}/${thirdName}]]`;
  return `> 相关笔记：${guide} · ${cheat} · ${third}`;
}

function configSection(data) {
  if (!data || typeof data !== 'object') {
    return '## 本地配置观察\n\n当前插件目录下没有可直接读取的 `data.json`，因此这里不推测个性化设置，建议以插件设置页和官方文档为准。';
  }
  const keys = Object.keys(data);
  const top = keys.slice(0, 12);
  const bullets = top.map(k => `- \`${k}\``).join('\n');
  return `## 本地配置观察\n\n从当前 vault 的本地配置可以看到，这个插件至少涉及以下设置键：\n\n${bullets}\n\n这些键说明此插件已经在当前 vault 中被实际配置过；如果你后续要深入整理，可优先从这些选项入手查看插件设置页。`;
}

function getUseCases(pluginId) {
  return sceneMap[pluginId] || ['在对应场景中启用插件功能', '结合你的笔记流程逐步尝试'];
}

function renderUseCases(pluginId, mode = 'full') {
  const items = getUseCases(pluginId);
  const picked = mode === 'compact' ? items.slice(0, 2) : items;
  return picked.map(item => `- ${item}`).join('\n');
}

function renderUseCaseLead(manifest, category) {
  return `这个插件更适合用于“${category}”相关流程，尤其适合下面这些真实使用场景：`;
}

function guideContent(folderName, baseName, manifest, data, type, category) {
  const title = `${baseName}插件使用指南`;
  const platform = manifest.isDesktopOnly ? '仅桌面端可用' : '桌面端与移动端都可能可用';
  const useCaseLead = renderUseCaseLead(manifest, category);
  const useCases = renderUseCases(manifest.id, 'full');
  const help = manifest.helpUrl ? `[官方文档](${manifest.helpUrl})` : 'manifest 中未提供 helpUrl';
  const templateNote = type === 'template'
    ? '这个插件适合配套“模板页”，因为它通常可以沉淀出固定写法、代码块、页面布局或可复用结构。'
    : '这个插件更适合配套“示例页”，因为它偏向界面增强、输入体验或行为优化，不一定有统一模板结构。';
...
function cheatContent(folderName, baseName, manifest, data, type, category) {
  const title = `${baseName}插件速查表`;
  const dataKeys = data && typeof data === 'object' ? Object.keys(data).slice(0, 10).map(k => `- \`${k}\``).join('\n') : '- 未检测到 `data.json`';
  return `${makeFrontmatter(title, [`${baseName} 速查表`, slugAlias(`${baseName} 插件速查表`)], ['obsidian', '插件', ...(extraTags[manifest.id] || []), '速查表'])}\n\n# ${title}\n\n${linkLine(folderName, baseName, type)}\n\n## 1. 基本信息\n\n- 名称：${manifest.name}\n- ID：\`${manifest.id}\`\n- 版本：${manifest.version}\n- 作者：${manifest.author || 'manifest 未声明'}\n- 平台：${manifest.isDesktopOnly ? '仅桌面端' : '桌面端 / 可能支持移动端'}\n- 分类：${category}\n\n## 2. 你最应该先做的事\n\n1. 打开设置页看插件选项\n2. 打开命令面板搜索插件名称\n3. 对照本地配置键确认当前 vault 已启用的能力\n4. 结合同目录的${type === 'template' ? '模板页' : '示例页'}试用\n\n## 3. 本地可见配置项\n\n${dataKeys}\n\n## 4. 用例速览\n\n${renderUseCases(manifest.id, 'compact')}\n\n## 5. 信息来源\n\n- \`manifest.json\`\n- ${data ? '`data.json`' : '未检测到 `data.json`'}\n- ${manifest.helpUrl ? `[官方说明页](${manifest.helpUrl})` : 'manifest 未提供 helpUrl'}\n\n## 6. 最小可用实践\n\n- 先在测试笔记中验证插件基本行为\n- 再决定是否要纳入正式工作流\n- 若涉及代码块、模板或可视化页面，可直接参考同目录配套文档`;
}

function exampleBlock(id, name) {
  const blocks = {
...
function thirdFileName(baseName, type, pluginId) {
  if (type === 'template') {
    return pluginId === 'obsidian-tasks-plugin' ? `${baseName}任务总览模板` : `${baseName}模板`;
  }
  return `${baseName}示例页`;
}

function thirdContent(folderName, baseName, manifest, data, type) {
  const thirdTitle = thirdFileName(baseName, type, manifest.id);
  const aliases = type === 'template' ? [`${baseName} 模板`, `${baseName} 使用模板`] : [`${baseName} 示例`, `${baseName} 演示页`];
  const roleTag = type === 'template' ? '模板' : '示例页';
  const intro = type === 'template'
    ? '这是一篇可直接复制和改造的模板页，适合你在自己的笔记里快速试用该插件。'
    : '这是一篇演示/示例页，适合你观察这个插件在实际笔记中的可能用法。';
  const note = type === 'template'
    ? '复制下面的结构到新笔记中，然后根据自己的目录、标签或字段继续调整。'
    : '建议你先在 Obsidian 中打开这篇示例页，再结合插件设置页和命令面板一起观察效果。';
  return `${makeFrontmatter(thirdTitle, aliases, ['obsidian', '插件', ...(extraTags[manifest.id] || []), roleTag])}\n\n# ${thirdTitle}\n\n${linkLine(folderName, baseName, type)}\n\n${intro}\n\n## 适用用例\n\n${renderUseCases(manifest.id, 'compact')}\n\n## 使用方式\n\n${note}\n\n## 示例内容\n\n${exampleBlock(manifest.id, manifest.name)}\n\n## 结合当前 vault 的建议\n\n- 如果你已经在某些笔记里实际使用过 **${manifest.name}**，建议把真实案例链接补到这里。\n- 如果本插件偏向界面增强而非内容结构，建议把你常用的设置截图或操作步骤补成自己的经验笔记。\n- 如果官方文档里有更完整的写法，可以在验证后再把正确语法同步回本页。\n\n## 本页说明\n\n- 插件 ID：\`${manifest.id}\`\n- 当前版本：${manifest.version}\n- data.json：${data ? '已检测到' : '未检测到'}\n- 官方说明：${manifest.helpUrl ? manifest.helpUrl : 'manifest 未提供'}\n`;
}

function loadState() {
  return safeReadJson(statePath) || { processed: {} };
}
...
function getPluginIds() {
  const ids = safeReadJson(communityPluginsPath);
  return Array.isArray(ids) ? ids : [];
}

function hasRequiredDocs(folderPath, baseName, type, pluginId) {
  const required = [
    `${baseName}插件使用指南.md`,
    `${baseName}插件速查表.md`,
    `${thirdFileName(baseName, type, pluginId)}.md`
  ];
  return required.every(name => fs.existsSync(path.join(folderPath, name)));
}

function processPlugin(pid, state) {
  const manifestPath = path.join(pluginDir, pid, 'manifest.json');
  const manifest = safeReadJson(manifestPath);
  if (!manifest) return { status: 'skipped', reason: 'missing-manifest', pluginId: pid };

  const data = safeReadJson(path.join(pluginDir, pid, 'data.json'));
  const baseName = sanitizeName(manifest.name || pid);
  const folderName = baseName;
  const folderPath = path.join(outDir, folderName);
  const category = inferCategory(manifest);
  const type = inferType(manifest, category);
  const stateKey = pid;
  const previous = state.processed[stateKey];
  const version = manifest.version || 'unknown';
  const manifestMtime = fs.statSync(manifestPath).mtimeMs;
  const docsPresent = hasRequiredDocs(folderPath, baseName, type, pid);
  const changed = fullRebuild || !previous || previous.version !== version || previous.folderName !== folderName || !docsPresent;

  if (forceRefresh) {
    return { status: 'skipped', reason: 'refresh-only', pluginId: pid, folderName };
  }

  if (!changed) {
    return { status: 'unchanged', pluginId: pid, folderName };
  }

  ensureDir(folderPath);
  const guideFile = path.join(folderPath, `${baseName}插件使用指南.md`);
  const cheatFile = path.join(folderPath, `${baseName}插件速查表.md`);
  const thirdBase = thirdFileName(baseName, type, pid);
  const thirdFile = path.join(folderPath, `${thirdBase}.md`);

  fs.writeFileSync(guideFile, guideContent(folderName, baseName, manifest, data, type, category), 'utf8');
  fs.writeFileSync(cheatFile, cheatContent(folderName, baseName, manifest, data, type, category), 'utf8');
  fs.writeFileSync(thirdFile, thirdContent(folderName, baseName, manifest, data, type), 'utf8');
  ...
  return { status: previous ? 'updated' : 'created', pluginId: pid, folderName, version };
}
...
function main() {
  ...
  for (const pid of pluginIds) {
    results.push(processPlugin(pid, state));
  }
  ...
  saveState(state);
  execFileSync(process.execPath, [indexScript], { stdio: 'inherit' });
  ...
  console.log(JSON.stringify({
    ...
    summary,
    changedPlugins: results.filter(item => item.status === 'created' || item.status === 'updated').map(item => item.pluginId)
  }, null, 2));
}
