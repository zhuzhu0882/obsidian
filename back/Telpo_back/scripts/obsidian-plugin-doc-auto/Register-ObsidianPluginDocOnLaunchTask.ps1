...
[CmdletBinding()]
param(
    [string]$TaskName = 'Obsidian-PluginDoc-OnLaunch',
...
if ([string]::IsNullOrWhiteSpace($WatcherScript)) {
    $WatcherScript = Join-Path $ScriptDirectory 'Start-ObsidianPluginDocOnObsidianLaunch.ps1'
}
...
$description = 'Watch for Obsidian launch and run the plugin documentation pipeline once per app start.'
